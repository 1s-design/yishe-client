...
const MAIN_IMAGE_DELETE_STANDARD_SELECTOR =
  '[class*="hoverBottomWrapper"] > div:nth-child(2)';
  ...
  try {
    return await page.evaluate(
      ({ fieldSelector, selectors }) => {
        ...
        const nodes = selectors.flatMap((selector) =>
          Array.from(field.querySelectorAll(selector)),
        );
        const uniqueKeys = new Set();
  ...
  try {
    return await page.evaluate((fieldSelector) => {
      ...
      const wrappers = Array.from(
        field.querySelectorAll('[class*="styles_imgWrapper__"]'),
      );
...
async function clickMainImageDeleteButtonByPreviewIndex(
  page,
  previewIndex,
  { hoverMode = "native" } = {},
) {
  try {
    return await page.evaluate(
      ({
        ...
        const toUniqueElements = (nodes) =>
          Array.from(
            new Set(nodes.filter((node) => node instanceof HTMLElement)),
          );

        const collectStandardDeleteButtons = (scope) => {
          if (!(scope instanceof HTMLElement)) return [];
          return toUniqueElements(
            Array.from(scope.querySelectorAll(deleteSelector)).filter((node) =>
              isVisible(node),
            ),
          );
        };
        ...
        const previewImages = toUniqueElements(
          previewSelectors.flatMap((selector) =>
            Array.from(field.querySelectorAll(selector)),
          ),
        ...
        if (!(targetImage instanceof HTMLImageElement)) {
          ...
          return {
            ...
            scopedDeleteCount: 0,
            deleteDescriptor: "",
            candidateDeleteDescriptors: fieldDeleteButtons
              .slice(0, 5)
        ...
        const fieldDeleteButtons = collectStandardDeleteButtons(field);
        const scopedDeleteButtons =
          collectStandardDeleteButtons(preferredScope);
    ...
    logger.warn(
      `抖店主图删除点击失败: previewIndex=${previewIndex}, error=${error?.message || error}`,
    );
    return {
      clicked: false,
      hoverMode,
...
async function clickDetailImageDeleteButtonByPreviewIndex(
  page,
  previewIndex,
  { hoverMode = "native" } = {},
) {
  try {
    return await page.evaluate(
      ({ fieldSelector, deleteSelector, previewIndex, hoverMode }) => {
        const field = document.querySelector(fieldSelector);
        const normalize = (value) =>
          String(value || "")
            .replace(/\s+/g, " ")
    ...
    logger.warn(
      `抖店详情图删除点击失败: previewIndex=${previewIndex}, error=${error?.message || error}`,
    );
    return {
      clicked: false,
      hoverMode,
...
async function clickOneMainImageDeleteButton(page) {
  try {
    return await page.evaluate(
      ({ fieldSelector, previewSelectors, deleteSelector }) => {
        const field = document.querySelector(fieldSelector);
        const normalize = (value) =>
          String(value || "")
            .replace(/\s+/g, " ")
        ...
        return {
          ...
          reason:
            previewImages.length > 0
              ? "standard-delete-button-not-found"
...
async function hoverAndClickOneMainImageDeleteButton(
  page,
  hoverMode = "native",
) {
  if (hoverMode === "js") {
    ...
    for (let index = 0; index < imageCount; index += 1) {
      ...
      try {
        ...
        const isPreviewImage =
          !!meta?.src &&
          (Number(meta?.width || 0) > 32 || Number(meta?.height || 0) > 32);
        ...
        const clickResult = await clickMainImageDeleteButtonByPreviewIndex(
          page,
          currentPreviewIndex,
          { hoverMode },
        );
        ...
        logger.warn(
          `抖店主图原生 hover 删除尝试失败: image[${index}], error=${error?.message || error}`,
        );
      }
    }
  } catch (error) {
  ...
  logger.info(
    "抖店商品主图清理：原生 hover 路径未成功，执行JS标准删除结果",
    fallbackResult,
  );
  return fallbackResult;
}

async function hoverAndClickOneDetailImageDeleteButton(
  page,
  hoverMode = "native",
) {
  if (hoverMode === "js") {
    return await clickDetailImageDeleteButtonByPreviewIndex(page, 0, {
      hoverMode,
    });
  }
  ...
  try {
    ...
    for (let index = 0; index < wrapperCount; index += 1) {
      ...
      try {
        ...
        const clickResult = await clickDetailImageDeleteButtonByPreviewIndex(
          page,
          currentPreviewIndex,
          { hoverMode },
        );
        ...
        logger.warn(
          `抖店详情图原生 hover 删除尝试失败: wrapper[${index}], error=${error?.message || error}`,
        );
      }
    }
  } catch (error) {
  ...
  const fallbackResult = await clickDetailImageDeleteButtonByPreviewIndex(
    page,
    0,
    { hoverMode: "js" },
  );
  logger.info(
    "抖店商品详情清理：原生 hover 路径未成功，执行JS标准删除结果",
    fallbackResult,
  );
  return fallbackResult;
}
...
async function waitForMainImagePreviewDecrease(page, beforeKeys) {
  try {
    await page.waitForFunction(
      ({ fieldSelector, selectors, previousKeys, previousCount }) => {
        const field = document.querySelector(fieldSelector);
        if (!(field instanceof HTMLElement)) {
          return true;
        }
        ...
        return (
          currentKeys.length < previousCount ||
...
async function waitForDetailImageWrapperDecrease(page, beforeKeys) {
  try {
    await page.waitForFunction(
      ({ fieldSelector, previousKeys, previousCount }) => {
        const field = document.querySelector(fieldSelector);
        if (!(field instanceof HTMLElement)) {
          return true;
        }
    ...
    logger.warn(
      `抖店详情图删除后未检测到 wrapper 减少: ${error?.message || error}`,
    );
    return false;
  }
}
...
async function clearMainImages(page, hoverMode = "native") {
  ...
  for (let round = 1; round <= 8; round += 1) {
    ...
    const clickResult = await hoverAndClickOneMainImageDeleteButton(
      page,
      hoverMode,
    );
    ...
    if (!clickResult?.clicked) {
      logger.warn(
        `抖店商品主图清理：未找到可点击删除按钮，remaining=${beforeKeys.length}, reason=${clickResult?.reason || "unknown"}`,
      );
      return {
        deletedCount,
    ...
    const removedCount =
      removedByCount > 0 ? removedByCount : hasRemovedKey ? 1 : 0;

    if (removedCount <= 0) {
      logger.warn(
        `抖店商品主图清理：点击删除后预览未减少，before=${beforeKeys.length}, after=${afterKeys.length}`,
      );
    ...
    logger.info(
      `抖店商品主图清理：删除成功，round=${round}, removed=${removedCount}, remaining=${afterKeys.length}`,
    );
  ...
  logger.info(
    `抖店商品主图清理结束: deleted=${deletedCount}, remaining=${remainingKeys.length}`,
  );
  return {
    deletedCount,
...
async function clearDetailImages(page, hoverMode = "native") {
  ...
  for (let round = 1; round <= 8; round += 1) {
    ...
    const clickResult = await hoverAndClickOneDetailImageDeleteButton(
      page,
      hoverMode,
    );
    ...
    if (!clickResult?.clicked) {
      logger.warn(
        `抖店商品详情清理：未找到可点击删除按钮，remaining=${beforeKeys.length}, reason=${clickResult?.reason || "unknown"}`,
      );
      return {
        deletedCount,
        remainingCount: beforeKeys.length,
    ...
    if (removedCount <= 0) {
      logger.warn(
        `抖店商品详情清理：点击删除后预览未减少，before=${beforeKeys.length}, after=${afterKeys.length}`,
      );
      await page.waitForTimeout(200);
      continue;
    }
    ...
    deletedCount += removedCount;
    logger.info(
      `抖店商品详情清理：删除成功，round=${round}, removed=${removedCount}, remaining=${afterKeys.length}`,
    );
  ...
  logger.info(
    `抖店商品详情清理结束: deleted=${deletedCount}, remaining=${remainingKeys.length}`,
  );
  return {
    deletedCount,
    remainingCount: remainingKeys.length,
...
async function clickWithFallback(
  ...
  try {
    ...
    logger.warn(
      `${successMessage}失败，尝试JS点击: ${error?.message || error}`,
    );
      ...
      return {
        ...
        fieldButtonCount:
          field instanceof HTMLElement
            ? field.querySelectorAll("button").length
            : 0,
...
async function clickDetailFillFromMainButton(page) {
  return await page.evaluate(
    ({ fieldSelector, text }) => {
      ...
      const documentMatches = collectMatches(document, "document").filter(
        (item) =>
          !fieldMatches.some((fieldItem) => fieldItem.node === item.node),
      );
      const allMatches = [...fieldMatches, ...documentMatches];
      const targetMeta =
        allMatches.find((item) => item.visible && item.enabled) ||
      ...
      if (!(targetMeta?.node instanceof HTMLElement)) {
        return {
          ...
          fieldButtonCount:
            field instanceof HTMLElement
              ? field.querySelectorAll(candidateSelector).length
              : 0,
...
async function resolveDetailFillFromMainButtonLocator(page) {
  const candidates = [
    page
      .locator(`${DETAIL_IMAGE_FIELD_SELECTOR} button`)
      .filter({ hasText: DETAIL_IMAGE_FILL_FROM_MAIN_TEXT }),
    page
      .locator(`${DETAIL_IMAGE_FIELD_SELECTOR} [role="button"]`)
      .filter({ hasText: DETAIL_IMAGE_FILL_FROM_MAIN_TEXT }),
    page
      .locator(`${DETAIL_IMAGE_FIELD_SELECTOR} .arco-btn`)
      .filter({ hasText: DETAIL_IMAGE_FILL_FROM_MAIN_TEXT }),
    page
      .locator(`${DETAIL_IMAGE_FIELD_SELECTOR} .semi-button`)
      .filter({ hasText: DETAIL_IMAGE_FILL_FROM_MAIN_TEXT }),
    page
      .locator(DETAIL_IMAGE_FIELD_SELECTOR)
      .getByText(DETAIL_IMAGE_FILL_FROM_MAIN_TEXT, { exact: false }),
    page
      .locator("button")
      .filter({ hasText: DETAIL_IMAGE_FILL_FROM_MAIN_TEXT }),
    page
  ...
  for (const candidate of candidates) {
    try {
      ...
      logger.warn(
        `抖店商品详情逻辑：解析“从主图填入”按钮 locator 失败: ${error?.message || error}`,
      );
    }
  }
...
async function waitForDetailFillFromMainReady(page) {
  await page.waitForFunction(
    ({ fieldSelector, selectors, text }) => {
      ...
      const fieldButtons =
        field instanceof HTMLElement
          ? Array.from(field.querySelectorAll(candidateSelector))
          : [];
      ...
      const button = [...fieldButtons, ...documentButtons].find((node) => {
        ...
        return (
          content.includes(normalizedText) && isVisible(node) && isEnabled(node)
        );
      });
...
async function checkLogin(page) {
  return await isShopPlatformLoggedIn(page, "doudian");
}
  ...
  try {
    return await page.evaluate(
      ({ successPatterns, failurePatterns }) => {
        ...
        const publishButton = buttons.find((item) =>
          item.text.includes("发布商品"),
        );

        return {
          matchedSuccessPatterns: successPatterns.filter((pattern) =>
            bodyText.includes(pattern),
          ),
          matchedFailurePatterns: failurePatterns.filter((pattern) =>
            bodyText.includes(pattern),
          ),
...
async function waitForDoudianPublishConfirmation(page, baselineSignals = null) {
  ...
  const baselineSuccessPatterns = new Set(
    baselineSignals?.matchedSuccessPatterns || [],
  );
  const baselineFailurePatterns = new Set(
    baselineSignals?.matchedFailurePatterns || [],
  );
  ...
  try {
    const result = await Promise.race([
      page
        .waitForFunction(
          ({ successPatterns, baselinePatterns }) => {
            const bodyText = String(document.body?.innerText || "").replace(
              /\s+/g,
              "",
            );
            const matched = successPatterns.find(
              (pattern) =>
                bodyText.includes(pattern) &&
      ...
      page
        .waitForFunction(
          ({ failurePatterns, baselinePatterns }) => {
            const bodyText = String(document.body?.innerText || "").replace(
              /\s+/g,
              "",
            );
            const matched = failurePatterns.find(
              (pattern) =>
                bodyText.includes(pattern) &&
                !baselinePatterns.includes(pattern),
            );
    ...
    const matchedFailure = finalSignals.matchedFailurePatterns.find(
      (pattern) => !baselineFailurePatterns.has(pattern),
    );
    if (matchedFailure) {
      throw new Error(`检测到失败提示: ${matchedFailure}`);
    }

    const matchedSuccess = finalSignals.matchedSuccessPatterns.find(
      (pattern) => !baselineSuccessPatterns.has(pattern),
    );
    if (matchedSuccess) {
      return {
        confirmed: true,
...
export async function publishToDoudian(publishInfo = {}) {
  ...
  try {
    ...
    const fileInputStartIndex = Number(
      settings.fileInputStartIndex ?? publishInfo.fileInputStartIndex ?? 2,
    );
    ...
    if (targetImages.length > 0) {
      ...
      logger.info(
        "抖店准备上传的图片路径:",
        preparedImages.filePaths.map(toUserFriendlyPath),
      );
      ...
      if (preparedImages.filePaths.length > 0) {
        ...
        logger.info(
          `抖店上传参数: requested=${preparedImages.filePaths.length}, preferredStartIndex=${fileInputStartIndex}`,
        );
        ...
        const startIndex =
          inputCount > fileInputStartIndex ? fileInputStartIndex : 0;
        ...
        logger.info(
          `抖店上传索引策略: startIndex=${startIndex}, usableInputCount=${Math.max(0, inputCount - startIndex)}, maxUploadCount=${Math.min(uploadFiles.length, 5)}`,
        );
        ...
        if (inputCount <= 0) {
          ...
          const snapshot = await firstInput
            .evaluate((el) => ({
              ...
              existingFiles: Array.from(el.files || []).map(
                (file) => file.name,
              ),
            }))
            .catch((error) => {
              logger.warn(
                `抖店读取首个文件输入框快照失败: input[0], error=${error?.message || error}`,
              );
              throw error;
            });
          ...
          const expectedNames = uploadFiles
            ...
            .filter(Boolean);
          logger.info(
            `抖店准备使用首个文件输入框批量上传: input[0], beforePreviewCount=${beforeCount}, files=${uploadFiles.map(toUserFriendlyPath)}`,
          );
          await firstInput.setInputFiles(uploadFiles);
          logger.info(
            `抖店首个文件输入框批量 setInputFiles 完成: input[0], count=${uploadFiles.length}`,
          );
          ...
          try {
            logger.info(
              `抖店等待单输入框接收多文件: input[0], expectedCount=${expectedNames.length}, files=${expectedNames}`,
            );
            ...
            await firstInput.evaluate((el, expected) => {
              return new Promise((resolve, reject) => {
                ...
                const check = () => {
                  const names = Array.from(el.files || []).map(
                    (file) => file.name,
                  );
                  const matched =
                    expected.length > 0 &&
                  ...
                  if (Date.now() - startedAt > 8000) {
                    return reject(
                      new Error(
                        `文件未完整进入 input, current=${names.join(", ")}`,
                      ),
                    );
                  }
            ...
            inputAccepted = true;
            logger.info(
              `抖店单输入框已接收多文件: input[0], files=${expectedNames}`,
            );
          } catch (error) {
          ...
          try {
            logger.info(
              `抖店等待单输入框批量上传确认: input[0], beforePreviewCount=${beforeCount}, files=${expectedNames}`,
            );
            await page.waitForFunction(
              (payload) => {
                const nodes = payload.selectors.flatMap((selector) =>
                  Array.from(document.querySelectorAll(selector)),
                );
                const uniqueKeys = new Set();
                ...
                const hasNewPreview = keys.some(
                  (key) => !payload.beforeKeys.includes(key),
                );
                return keys.length > payload.beforeCount || hasNewPreview;
              },
              { beforeCount, beforeKeys, selectors: PREVIEW_IMAGE_SELECTORS },
              { timeout: 15000 },
            );
            uploadConfirmed = true;
            const afterCount = await countUploadedPreviewImages(page);
            logger.info(
              `抖店单输入框批量上传预览确认成功: input[0], before=${beforeCount}, after=${afterCount}, files=${expectedNames}`,
            );
          } catch (error) {
            logger.warn(
              `抖店单输入框批量上传预览确认超时: input[0], before=${beforeCount}, expectedIncrease=${uploadFiles.length}, error=${error?.message || error}`,
            );
            await page.waitForTimeout(3000);
            const afterKeys = await collectUploadedPreviewKeys(page);
            const afterCount = afterKeys.length;
            const hasNewPreview = afterKeys.some(
              (key) => !beforeKeys.includes(key),
            );
            if (afterCount > beforeCount || hasNewPreview) {
              uploadConfirmed = true;
              logger.info(
                `抖店单输入框批量上传超时后补充确认成功: input[0], before=${beforeCount}, after=${afterCount}, hasNewPreview=${hasNewPreview}`,
              );
            }
          }
          ...
          if (!inputAccepted && uploadConfirmed) {
            logger.info(
              `抖店单输入框文件确认已跳过: input[0], 原因=${inputAcceptErrorMessage || "input.files 已被页面清空，但预览已确认上传成功"}`,
            );
          }
          ...
          if (!inputAccepted && !uploadConfirmed) {
            if (inputAcceptErrorMessage) {
              logger.warn(
                `抖店单输入框接收多文件未确认: input[0], error=${inputAcceptErrorMessage}`,
              );
            }
          ...
          logger.info(
            `抖店上传流程结束: uploaded=${uploadResult.uploadedPaths.length}/${uploadResult.requested}`,
          );
        }
      } else {
    ...
    try {
      ...
      if ((clearResult?.remainingCount || 0) > 0) {
        throw new Error(
          `旧详情图未清理干净，remaining=${clearResult.remainingCount}`,
        );
      }
      ...
      logger.info(
        "抖店商品详情逻辑：“从主图填入”按钮等待前快照",
        await inspectDetailFillFromMainButton(page),
      );
      try {
        await waitForDetailFillFromMainReady(page);
      } catch (error) {
        logger.warn(
          `抖店商品详情逻辑：“从主图填入”按钮等待就绪超时，继续尝试点击: ${error?.message || error}`,
        );
        logger.info(
          "抖店商品详情逻辑：“从主图填入”按钮等待失败快照",
          await inspectDetailFillFromMainButton(page),
        );
      }
      await page.waitForTimeout(200);
      ...
      await clickWithFallback(
        ...
        async () => {
          ...
          logger.info(
            "抖店商品详情逻辑：“从主图填入”按钮JS点击结果",
            fillButtonClickResult,
          );
    ...
    try {
        ...
        () =>
          page.evaluate(() => {
            ...
            const target = buttons.find((button) =>
              String(button.textContent || "").includes("发布商品"),
            );
            if (!(target instanceof HTMLElement)) return false;
            target.click();
            return true;
      ...
      const publishConfirmation = await waitForDoudianPublishConfirmation(
        page,
        publishBaselineSignals,
      );
      ...
      publishFinalUrl = String(
        publishConfirmation?.finalUrl || page.url() || "",
      ).trim();
      logger.info(
        `抖店发布提交流程：已确认发布成功，signal=${publishSuccessSignal || "unknown"}, detail=${publishSuccessDetail || "n/a"}, finalUrl=${publishFinalUrl || "n/a"}`,
      );
    } catch (error) {
