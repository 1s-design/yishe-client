...
import { normalizeBoolean, normalizeTemuSettings, pushTrace } from "./utils.js";
import {
  ensureTemuLoginPage,
  performTemuLogin,
...
const TEMU_PUBLISH_DETAIL_PAGE_URL =
  "https://agentseller.temu.com/goods/edit?from=productList&productId=";
const TEMU_PUBLISH_DETAIL_REQUEST_URL =
  "https://agentseller.temu.com/visage-agent-seller/product/edit";
...
function isTemuHttpOnlyCookie(name = "") {
  ...
  return TEMU_SESSION_HTTP_ONLY_PATTERNS.some((pattern) =>
...
function buildTemuSessionRestoreSettings(input = {}) {
  ...
  return {
    ...
    activatePage:
      normalizeBoolean(input?.activatePage) ||
      normalizeBoolean(input?.foreground),
...
function buildTemuSessionRestoreCookiePlan(settings) {
  ...
  for (const [regionKey, target] of Object.entries(
    TEMU_SESSION_RESTORE_REGION_TARGETS,
  )) {
    const regionRecord = normalizeTemuSessionRestoreRegion(
      settings?.session?.[regionKey],
    );
    const cookieEntries = Object.entries(regionRecord.cookies)
      .filter(
        ([name, value]) =>
          String(name || "").trim() && value !== undefined && value !== null,
      )
      .map(([name, value]) => ({
...
function buildTemuCapturedRequest(request, matchedKeyword = "") {
  ...
  if (
    !rawPostData &&
    postDataBuffer &&
    Buffer.isBuffer(postDataBuffer) &&
    postDataBuffer.length
  ) {
...
function buildTemuCapturedRequestFromCdp(
  event = {},
  matchedKeyword = "",
  resolvedPostData = "",
) {
  ...
  const requestUrl = String(request?.url || "").trim();
  const rawPostData = String(
    resolvedPostData || request?.postData || "",
  ).trim();
  const normalizedBody = normalizeRequestBody(rawPostData);
...
async function createTemuMatchedRequestCapture(pageOrContext, options = {}) {
  const page =
    pageOrContext && typeof pageOrContext?.context === "function"
      ? pageOrContext
      : null;
  const context = page ? page.context() : pageOrContext;
  const requestKeywords = normalizeStringList(
    options?.requestKeywords || options?.requestKeyword,
  );
  ...
  const pushMatchedRequest = (matchedRequest) => {
    ...
    const existingIndex = state.matchedRequests.findIndex((item) => {
      return (
        String(item?.method || "")
          .trim()
          .toUpperCase() ===
          String(matchedRequest?.method || "")
            .trim()
            .toUpperCase() &&
        String(item?.url || "").trim() ===
          String(matchedRequest?.url || "").trim()
      );
    });
    ...
    if (existingIndex >= 0) {
      ...
      if (
        (!isTemuBodyRequest(existingRequest) &&
          isTemuBodyRequest(matchedRequest)) ||
        (!isTemuMutationRequest(existingRequest) &&
          isTemuMutationRequest(matchedRequest))
      ) {
  ...
  const onRequest = (request) => {
    try {
      ...
      const matchedRequest = buildTemuCapturedRequest(
        request,
        matchedKeyword.raw,
      );
  ...
  const onCdpRequestWillBeSent = async (event) => {
    try {
      ...
      if (
        resourceType &&
        !TEMU_REQUEST_CAPTURE_RESOURCE_TYPES.includes(resourceType)
      ) {
        return;
      }
      ...
      if (
        ...
        try {
          const postDataResponse = await cdpSession.send(
...
function pickPreferredTemuMatchedRequest(matchedRequests = []) {
  const normalizedRequests = Array.isArray(matchedRequests)
    ? matchedRequests
    : [];
  ...
  return (
    normalizedRequests.find(
      (item) => isTemuMutationRequest(item) && isTemuBodyRequest(item),
    ) ||
...
async function waitForTemuMatchedRequest(captureState, timeoutMs = 60_000) {
  ...
  while (Date.now() < deadline) {
    ...
    if (matchedRequests.length) {
      const preferredMatchedRequest =
        pickPreferredTemuMatchedRequest(matchedRequests);
      if (preferredMatchedRequest) {
        fallbackMatchedRequest = preferredMatchedRequest;
      }

      if (
        preferredMatchedRequest &&
        (isTemuBodyRequest(preferredMatchedRequest) ||
          isTemuMutationRequest(preferredMatchedRequest))
      ) {
        return {
          success: true,
...
async function clickTemuTriggerByText(
  page,
  texts = [],
  timeoutMs = TEMU_REQUEST_CAPTURE_CLICK_TIMEOUT,
) {
  const deadline = Date.now() + timeoutMs;
...
async function clickTemuTriggerBySelector(
  page,
  selector = "",
  timeoutMs = TEMU_REQUEST_CAPTURE_CLICK_TIMEOUT,
) {
...
async function triggerTemuPublishDetailRequestCapture(page, settings = {}) {
  ...
  if (settings.triggerMode === TEMU_PUBLISH_DETAIL_TRIGGER_MODE_BUTTON_TEXT) {
    return await clickTemuTriggerByText(
      page,
      settings.buttonTexts,
      TEMU_REQUEST_CAPTURE_CLICK_TIMEOUT,
    );
  }

  if (settings.triggerMode === TEMU_PUBLISH_DETAIL_TRIGGER_MODE_SELECTOR) {
    return await clickTemuTriggerBySelector(
      page,
...
function buildCompactTemuSessionBundle(sessionBundle = {}) {
  return {
    ...
    mallList: Array.isArray(sessionBundle?.mallList)
    ...
    cookies_global:
      sessionBundle?.cookies_global || sessionBundle?.cookies || {},
    cookies_us: sessionBundle?.cookies_us || {},
    cookies_eu: sessionBundle?.cookies_eu || {},
  };
}

function buildTemuSessionFailureDebug(sessionResult = {}, options = {}) {
  const sessionBundle = sessionResult?.sessionBundle || null;
  if (!sessionBundle) {
    return {};
  }
  ...
  return {
    ...
    sessionWarnings: Array.isArray(sessionBundle?.warnings)
      ? sessionBundle.warnings
      : [],
    requestCapture: sessionBundle?.requestCapture || {},
    regionCollection: sessionBundle?.regionCollection || {},
    partialSessionBundle: buildCompactTemuSessionBundle(sessionBundle),
  };
}

function buildTemuSessionAcquireFailureMessage(
  sessionResult = {},
  loginState = {},
) {
  if (sessionResult.reason !== "login_required") {
    return sessionResult.message || `${PLATFORM_NAME}会话获取失败`;
  }

  if (loginState?.requiresAuthentication) {
    return `请先处理${PLATFORM_NAME}登录验证后再重新获取会话`;
  }

  return sessionResult.message
    ? `${PLATFORM_NAME}已完成登录跳转，但会话校验仍失败：${sessionResult.message}`
    : `${PLATFORM_NAME}已完成登录跳转，但会话校验仍失败，请查看缺失项后重新获取会话`;
}
...
function buildTemuSessionAcquireSettings(input = {}) {
  return {
    ...
    keepPageOpen: normalizeKeepPageOpen(input?.keepPageOpen),
    collectRegionCookies: normalizeCollectRegionCookies(
      input?.collectRegionCookies,
    ),
    includeDebugInfo: normalizeIncludeDebugInfo(input?.includeDebugInfo),
  };
}

function hasTemuLoginCredentials(settings = {}) {
  return (
    !!String(settings?.account || "").trim() &&
    !!String(settings?.password || "").trim()
  );
}

async function fetchTemuStoredSessionCredentials(profileId = "") {
  if (!profileId) return null;
  const token = String(getTokenValue?.() || "").trim();
  if (!token) {
    logger.warn(`${PLATFORM_NAME}自动加载已存储凭证失败：缺少 token`, {
      profileId,
    });
    return null;
  }
  try {
    const response = await fetch(
      `${TEMU_REMOTE_API_BASE}/user/getPlatformSessions`,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ platform: "temu", profileId }),
      },
    );
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      logger.warn(`${PLATFORM_NAME}自动加载已存储凭证失败`, {
        profileId,
        status: response.status,
      });
      return null;
    }
    // NestJS TransformInterceptor 包裹为 { data: {...}, code: 0, ... }
    const data =
      payload?.data &&
    ...
    const account = String(data?.account || session?.account || "").trim();
    const password = String(data?.password || session?.password || "").trim();
    if (!account || !password) {
      logger.warn(
        `${PLATFORM_NAME}自动加载已存储凭证：API 返回中缺少账号密码`,
        {
          profileId,
          hasAccount: !!account,
    ...
    logger.warn(`${PLATFORM_NAME}自动加载已存储凭证异常`, {
      profileId,
...
export async function runTemuLoginSmallFeature(
  input = {},
  runtimeOptions = {},
) {
  ...
  try {
    ...
    pushTrace(
      executionTrace,
      "check_login_state_before",
      loginStateBefore.loggedIn ? "success" : "pending",
      loginStateBefore,
    );
    ...
    pushTrace(
      executionTrace,
      "check_login_state_after",
      loginStateAfter.loggedIn ? "success" : "pending",
      loginStateAfter,
    );
    ...
    return {
      success: true,
      message:
        loginResult.reason === "already_logged_in"
          ? `${PLATFORM_NAME}当前已登录`
    ...
    if (managePage && page && !settings.keepPageOpen) {
      try {
        ...
        logger.warn(
          `${PLATFORM_NAME}工具关闭页面失败: ${closeError?.message || closeError}`,
        );
      }
...
export async function runTemuSessionAcquireSmallFeature(
  input = {},
  runtimeOptions = {},
) {
  const profileId = String(input?.profileId || "").trim() || undefined;
  ...
  try {
    ...
    // 如果页面在 about:blank，导航到 seller 首页检测登录状态
    const currentUrl = String(page.url() || "");
    if (!currentUrl || currentUrl === "about:blank") {
      logger.info(
        `${PLATFORM_NAME}页面在 about:blank，导航到 seller 首页检测登录`,
      );
    ...
    const loginStateBefore = await resolveTemuLoginState(page);
    logger.info(`${PLATFORM_NAME}登录检测结果`, {
      loggedIn: loginStateBefore.loggedIn,
      currentUrl: page.url(),
      reason: loginStateBefore.reason,
    });
    pushTrace(
      executionTrace,
      "check_login_state_before",
    ...
    // 如果 input 中没有账号密码，但用户在服务端已存储凭证，则从服务端自动加载
    if (!hasTemuLoginCredentials(settings) && profileId) {
      logger.info(`${PLATFORM_NAME}尝试从服务端自动加载已存储凭证`, {
        profileId,
      });
      const storedCreds = await fetchTemuStoredSessionCredentials(profileId);
      if (storedCreds) {
        settings.account = storedCreds.account;
        settings.password = storedCreds.password;
        logger.info(`${PLATFORM_NAME}已从服务端自动加载存储凭证`, {
          hasAccount: !!settings.account,
        });
        pushTrace(executionTrace, "auto_load_credentials", "success", {
          profileId,
          hasAccount: !!settings.account,
        });
      } else {
        logger.info(`${PLATFORM_NAME}服务端未找到已存储凭证，继续原有流程`);
        pushTrace(executionTrace, "auto_load_credentials", "skipped", {
          profileId,
    ...
    const credentialsReady = hasTemuLoginCredentials(settings);
    logger.info(`${PLATFORM_NAME}凭证检查`, {
      hasAccount: !!settings.account,
      hasPassword: !!settings.password,
      credentialsReady,
      loggedIn: loginStateBefore.loggedIn,
    ...
    // 仅当已登录时才尝试直接采集会话；未登录时不调用 collectTemuSessionBundle
    // 避免无谓跳转到 kuajingmaihuo.com 破坏当前页面状态
    if (loginStateBefore.loggedIn) {
      logger.info(`${PLATFORM_NAME}会话获取步骤：已登录，尝试直接采集会话`, {
        currentUrl: page.url(),
      });
      sessionResult = await collectTemuSessionBundle(page, {
        collectRegionCookies: settings.collectRegionCookies,
      });
    }

    // 判断是否需要登录：初始未登录 或 collectTemuSessionBundle 返回 login_required
    const needsLogin =
      !loginStateBefore.loggedIn ||
      (sessionResult && sessionResult.reason === "login_required");

    if (
      sessionResult &&
      !sessionResult.success &&
      sessionResult.reason === "login_required" &&
      !credentialsReady
    ) {
      pushTrace(executionTrace, "require_login_credentials", "failed", {
      ...
      return {
        success: false,
        message: `当前环境未登录${PLATFORM_NAME}，请录入账号密码后重新采集，或者先在浏览器环境中手动登录`,
        data: {
          ...
          detectedButtons: snapshot.buttons,
          detectedInputs: snapshot.inputs,
          ...buildTemuSessionFailureDebug(sessionResult, settings),
          executionTrace,
          pageKeptOpen: settings.keepPageOpen,
        },
      };
    ...
    // 未调用 collectTemuSessionBundle（初始就已检测到未登录）且无凭证
    if (!sessionResult && needsLogin && !credentialsReady) {
      pushTrace(executionTrace, "require_login_credentials", "failed", {
        reason: loginStateBefore.reason || "",
        currentUrl: page.url(),
      });
    ...
    if (needsLogin && credentialsReady) {
      const loginResult = await performTemuLogin(page, settings, pageOperator);
      if (!loginResult.success) {
        pushTrace(executionTrace, "perform_login", "failed", {
          reason: loginResult.reason,
        ...
        return {
          success: false,
          message:
            loginResult.message || `${PLATFORM_NAME}登录失败，无法继续获取会话`,
          data: {
            featureKey: "temu-session-acquire",
            profileId: profileId || null,
            currentUrl: page.url(),
    ...
    if (!sessionResult.success) {
      ...
      const snapshot = await collectTemuFrameworkSnapshot(page);
      const loginStateAfterFailure = await resolveTemuLoginState(page);
      const failureMessage = buildTemuSessionAcquireFailureMessage(
        sessionResult,
        loginStateAfterFailure,
      );
      return {
        success: false,
...
export async function runTemuSessionRestoreSmallFeature(
  input = {},
  runtimeOptions = {},
) {
  ...
  try {
    ...
    if (!settings.validateAfterRestore) {
      pushTrace(
        executionTrace,
        "skip_login_validation_after_restore",
        "success",
        {
    ...
    pushTrace(
      executionTrace,
      "check_login_state_after_restore",
      loginState.loggedIn ? "success" : "pending",
      loginState,
    );
    ...
    if (
      managePage &&
      page &&
      (!settings.keepPageOpen || !settings.validateAfterRestore)
    ) {
      try {
        await page.close();
      } catch (closeError) {
...
export async function runTemuPublishDetailRequestCaptureSmallFeature(
  input = {},
  runtimeOptions = {},
) {
  ...
  try {
    ...
    const triggerResult = await triggerTemuPublishDetailRequestCapture(
      page,
      settings,
    );
    pushTrace(
    ...
    if (!captureResult.success) {
      ...
      pushTrace(executionTrace, "capture_target_request", "failed", {
        ...
        lastObservedRequestUrl:
          requestCapture.state.lastObservedRequestUrl || "",
      });
    ...
    return {
      ...
      data: {
        ...
        capturedAt:
          captureResult.matchedRequest?.capturedAt || new Date().toISOString(),
        pageKeptOpen: settings.keepPageOpen,
      },
    };
  } catch (error) {
