...
const TEMU_IMAGE_PICKER_CONFIRM_LABELS = [
  "确定",
  "确认",
  "完成",
  "选择",
...
function resolveTemuStoredCredentialPair(record = {}) {
  ...
  const session = isPlainObject(normalizedRecord?.session)
...
function inferMimeType(filePath = "") {
  const extension = extname(String(filePath || ""))
    .replace(".", "")
    .toLowerCase();
  switch (extension) {
    case "png":
...
function buildTemuUploadHeaderCandidates(
  ...
  const cookieHeader = normalizeText(
    sessionContext.cookieHeader ||
      buildCookieHeader(sessionContext.cookies || {}),
  );
  const baseHeaders = {
    accept: "application/json, text/plain, */*",
    cookie: cookieHeader,
    origin: normalizeText(
      requestCaptureState.origin ||
        sessionContext.headers?.origin ||
  ...
  const userAgent = normalizeText(
    requestCaptureState.userAgent ||
      sessionContext.userAgent ||
      sessionContext.headers?.["user-agent"],
  );
  const mallId = normalizeText(
    requestCaptureState.mallId ||
      sessionContext.mallId ||
      sessionContext.headers?.mallid,
  );
  ...
  return {
    ...
    headerCandidates: headerCandidates.filter((headers, index, list) => {
      return (
        list.findIndex(
          (item) => JSON.stringify(item) === JSON.stringify(headers),
        ) === index
      );
    }),
  };
...
function normalizeTemuUploadSessionContext(sessionContext = {}) {
  ...
  const cookies =
    sessionContext.cookies && typeof sessionContext.cookies === "object"
      ? sessionContext.cookies
      : {};
  const headers =
    sessionContext.headers && typeof sessionContext.headers === "object"
      ? sessionContext.headers
      : sessionContext.headersTemplate &&
          typeof sessionContext.headersTemplate === "object"
        ? sessionContext.headersTemplate
        : {};
  const cookieHeader = normalizeText(
  ...
  return {
    ...
    userAgent: normalizeText(
      sessionContext.userAgent || headers["user-agent"] || "",
    ),
    mallId: normalizeText(sessionContext.mallId || headers.mallid || ""),
    antiContent: normalizeText(
      sessionContext.antiContent || headers["anti-content"] || "",
    ),
    cookieCount:
      Number.isFinite(cookieCount) && cookieCount > 0
        ? cookieCount
        : Object.keys(cookies).length,
    effectiveRegion:
      normalizeText(sessionContext.effectiveRegion || "global") || "global",
  };
}

export function resolveTemuStoredSessionContext(
  publishInfo = {},
  preferredSessionContext = null,
) {
  const settings =
...
function buildTemuRealtimeStoredSessionContext(profileData = {}) {
  ...
  return normalizeTemuUploadSessionContext({
    ...
    account: normalizeText(profileRecord.account || session?.account),
    password: normalizeText(profileRecord.password || session?.password),
    currentUrl: normalizeText(
...
export async function resolveTemuRealtimeSessionContext(
  publishInfo = {},
  preferredSessionContext = null,
) {
  const directSession = resolveTemuStoredSessionContext(
    publishInfo,
...
export async function resolveTemuLoginCredentials(
  publishInfo = {},
  preferredSessionContext = null,
) {
  ...
  const storedProfile = isPlainObject(preferredSessionContext?.storedProfile)
    ...
    : null;
  const storedProfileCredentials =
    resolveTemuStoredCredentialPair(storedProfile);
...
function buildMergedRegionSession(
  nextCookies = {},
  nextHeaders = {},
  fallbackSession = {},
  updatedAt = "",
) {
  const fallbackCookies = isPlainObject(fallbackSession?.cookies)
    ? fallbackSession.cookies
    : {};
  const fallbackHeaders = isPlainObject(fallbackSession?.headers)
    ? fallbackSession.headers
    : {};
...
function buildTemuStoredSessionUpdatePayload(
  sessionBundle = {},
  profileId = "",
  existingProfile = {},
  credentials = null,
  ...
  const collectedAt =
    normalizeText(sessionBundle?.collectedAt || new Date().toISOString()) ||
    new Date().toISOString();
  const currentProfile = isPlainObject(existingProfile) ? existingProfile : {};
  const currentSession = isPlainObject(currentProfile?.session)
    ? currentProfile.session
    : {};
  const currentUserInfo = isPlainObject(currentProfile?.userInfo)
  ...
  const nextGlobalHeaders = Object.keys(headersTemplate).length
    ...
    : isPlainObject(regionHeaders?.global)
      ...
      : isPlainObject(currentProfile?.headersTemplate)
        ? currentProfile.headersTemplate
        : {};
  const mallList =
    Array.isArray(sessionBundle?.mallList) && sessionBundle.mallList.length
      ...
      : Array.isArray(currentProfile?.mallList)
        ...
        : Array.isArray(currentUserInfo?.mallList)
          ? currentUserInfo.mallList
          : [];
  const mallId = normalizeText(
    sessionBundle?.mallId || currentProfile?.mallId || currentUserInfo?.mallId,
  );
  const mallName = normalizeText(
    sessionBundle?.mallName ||
      currentProfile?.mallName ||
      currentUserInfo?.mallName,
  );
  const accountId = normalizeText(
    sessionBundle?.accountId ||
...
async function persistTemuCollectedSessionToServer(
  sessionBundle = {},
  profileId = "",
...
export async function resolveTemuValidatedSessionContext(
  publishInfo = {},
  options = {},
) {
  ...
  if (
    (!collectedSessionResult?.success ||
      !collectedSessionResult.sessionBundle) &&
    collectedSessionResult?.reason === "login_required"
  ) {
    ...
    if (
      ...
      logger.info(
        `${PLATFORM_NAME}现场采集检测到未登录，准备使用已存账号密码自动登录`,
        {
          profileId: profileId || "",
          credentialSource:
            resolvedLoginCredentials.source || "stored_platform_session",
        },
      );
      ...
      if (!loginResult?.success) {
        return {
          ...
          message:
            loginResult?.message ||
            collectedSessionResult?.message ||
            "Temu 自动登录失败",
        };
      }
  ...
  let persisted = false;
  let persistMessage = "";
  const storedProfile = isPlainObject(realtimeSession?.storedProfile)
    ? realtimeSession.storedProfile
    : null;
  logger.info(`${PLATFORM_NAME}现场采集会话成功`, {
    profileId: profileId || "",
    cookieCount: Object.keys(
      collectedSessionResult.sessionBundle?.cookies || {},
    ).length,
  });
...
function normalizeUploadedImageRecord(payload = {}, fileEntry = {}) {
  ...
  const widthValue =
    payload?.width ?? payload?.result?.width ?? payload?.data?.width;
  const heightValue =
    payload?.height ?? payload?.result?.height ?? payload?.data?.height;
...
async function uploadSingleTemuImage(fileEntry, headerCandidates = []) {
  ...
  return {
    ...
    message:
      lastError?.message ||
      `上传图片失败: ${fileEntry.fileName || fileEntry.source || ""}`,
    attemptDetails,
  };
}
...
async function refreshTemuUploadSession(page, options = {}) {
  const fallbackHeadersTemplate = {
    origin:
      options.requestCaptureState?.origin || "https://agentseller.temu.com",
    referer:
...
export function createTemuLiveRequestCapture(context) {
  ...
  const onRequest = (request) => {
    try {
      ...
      if (
        normalizedHeaders.mallid &&
        normalizedHeaders.mallid !== "undefined"
      ) {
      ...
      ({ triggerLabels, sectionKeywords }) => {
        ...
        return clickables
          .map((element) => {
            ...
            const matchedLabel = triggerLabels.find((label) =>
              text.includes(label),
            );
            if (!matchedLabel) {
              return null;
          ...
          .sort((left, right) => {
            ...
            return (
              leftSectionScore - rightSectionScore ||
...
async function selectTemuImagesInPicker(
  page,
  uploadedImages = [],
  ...
  return page
    .evaluate(
      ({ matchers, maxCount }) => {
        ...
        enrichedContainers.sort(
          (left, right) => right.imageCount - left.imageCount,
        );
        const activeContainer =
          enrichedContainers[0]?.container || document.body;

        const cards = [];
        const cardSet = new Set();
        for (const img of Array.from(
          activeContainer.querySelectorAll("img"),
        ).filter(isVisible)) {
          ...
          const score = matchers.reduce((acc, matcher) => {
            ...
            return normalizedMatcher && haystack.includes(normalizedMatcher)
              ? acc + 1
              : acc;
          }, 0);
        ...
        const preferredCards = cards.filter(
          (item) => !item.selected && item.score > 0,
        );
        ...
        return {
          ...
          usedFallback:
            preferredCards.length < Math.min(cards.length, maxCount),
        };
      },
      {
...
async function confirmTemuImagePicker(page) {
  const clicked = await clickClickableByText(
    page,
    TEMU_IMAGE_PICKER_CONFIRM_LABELS,
    {
...
export async function bindTemuUploadedImagesToEditPage(
  page,
  uploadedImages = [],
) {
  const images = Array.isArray(uploadedImages)
    ? uploadedImages.filter((item) => item?.url)
    : [];
  if (!images.length) {
    return {
      success: true,
  ...
  for (const candidate of triggerCandidates) {
    ...
    for (let pickerAttempt = 0; pickerAttempt < 2; pickerAttempt += 1) {
      ...
      await page.waitForTimeout(1_500);
      const selectionResult = await selectTemuImagesInPicker(
        page,
        images,
        images.length,
      );
      ...
      attemptResults.push({
        ...
        reason: confirmResult.success
          ? ""
          : confirmResult.reason || "picker_confirm_failed",
        selectionResult,
        confirmResult,
      });
...
export async function uploadTemuPublishImages(
  page,
  publishInfo = {},
  options = {},
) {
  const resolvedSession = await resolveTemuValidatedSessionContext(
    publishInfo,
    {
  ...
  return await uploadTemuImagesToCloud(
    ...
    resolveTemuPublishImageSources(publishInfo),
    {
      ...
      emptyMessage:
        ...
        `${PLATFORM_NAME}发布数据未提供图片，跳过图片上传`,
    },
  );
}

export async function uploadTemuImagesToCloud(
  page,
  imageSources = [],
  options = {},
) {
  ...
  const emptyMessage =
    normalizeText(options.emptyMessage || "") ||
  ...
  let sessionContext =
    normalizeTemuUploadSessionContext(options.sessionContext) ||
  ...
  if (!sessionContext?.success) {
    ...
    return {
      ...
      message:
        ...
        `获取当前 Temu 会话失败，无法上传${resourceLabel}`,
      requestedImageCount: normalizedImageSources.length,
  ...
  try {
    ...
    for (let index = 0; index < preparedResult.fileEntries.length; index += 1) {
      ...
      logger.info(
        `${PLATFORM_NAME}开始上传${resourceLabel}到Temu云文件 ${current}/${total}`,
        {
          source: fileEntry.source,
      ...
      for (
        let uploadAttempt = 0;
        uploadAttempt < TEMU_UPLOAD_RETRY_LIMIT;
        uploadAttempt += 1
      ) {
        ...
        const canRefreshSession =
          uploadAttempt < TEMU_UPLOAD_RETRY_LIMIT - 1 &&
        ...
        if (!refreshedSessionContext?.success) {
          retryEvents.push({
            ...
            refreshed: false,
            message:
              refreshedSessionContext?.message || "refresh_session_failed",
          });
          break;
        }
        ...
        activeHeaderCandidates =
          rebuiltHeaders.headerCandidates || activeHeaderCandidates;
        retryEvents.push({
          source: fileEntry.source,
          fileName: fileEntry.fileName,
          uploadAttempt: uploadAttempt + 1,
      ...
      if (!uploadResult?.success) {
        ...
        logger.error(
          `${PLATFORM_NAME}${resourceLabel}上传到Temu云文件失败 ${current}/${total}`,
          {
            source: fileEntry.source,
            sourceName: summarizeImageSourceForLog(fileEntry.source),
            fileName: fileEntry.fileName,
      ...
      logger.info(
        `${PLATFORM_NAME}${resourceLabel}上传到Temu云文件成功 ${current}/${total}`,
        {
          source: fileEntry.source,
          sourceName: summarizeImageSourceForLog(fileEntry.source),
          fileName: fileEntry.fileName,
  ...
  const success =
    ...
    uploadedImages.length === normalizedImageSources.length;
  ...
  if (success) {
    logger.info(
      `${PLATFORM_NAME}${resourceLabel}上传到Temu云文件完成`,
      summaryPayload,
    );
  } else {
  ...
  return {
    success,
    message: success
      ? `${PLATFORM_NAME}${resourceLabel}上传完成`
      : `${PLATFORM_NAME}${resourceLabel}上传未完成`,
    requestedImageCount: normalizedImageSources.length,
    requestedSourceCount: normalizedImageSources.length,
    uploadedCount: uploadedImages.length,
    uploadedImages,
