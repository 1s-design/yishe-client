...
import { inspectShopPlatformLogin } from "../shopLoginFeatures.js";

const PLATFORM_KEY = "doudian";
const PLATFORM_NAME = "抖店";
...
async function collectDoudianUserInfo(page) {
  return await page
    .evaluate(() => {
      ...
      const readStorage = (storage) => {
        ...
        for (let index = 0; index < storage.length; index += 1) {
          ...
          if (!key) continue;
          if (!/(user|account|shop|store|merchant|operator|seller|login)/i.test(key)) {
            continue;
          }
      ...
      const globals = {
        ...
        slardarContext: window.Slardar?.context || null,
      };
    ...
    .then((snapshot) => {
      const candidates = [
        ...
        snapshot?.globals?.slardarContext,
...
export async function runDoudianSessionAcquireSmallFeature(input = {}, runtimeOptions = {}) {
  const platformConfig = PLATFORM_CONFIGS?.[PLATFORM_KEY];
  if (!platformConfig) {
    throw new Error("抖店平台配置不存在");
  }
  ...
  try {
    ...
    if (!loginResult.isLoggedIn) {
      return {
        ...
        data: {
          featureKey: "doudian-session-acquire",
    ...
    const browserCookies = await context.cookies([
      ...
      "https://login.jinritemai.com",
      "https://seller.jinritemai.com",
      "https://www.douyin.com",
    ]);
    const cookies = buildCookieMap(browserCookies);
    const userInfo = await collectDoudianUserInfo(page);
    ...
    const sessionBundle = {
      ...
      validation: {
        status: Object.keys(cookies).length && (hasIdentity || hasShop) ? "fresh" : "incomplete",
