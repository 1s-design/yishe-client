...
const DEFAULT_CREATE_URL =
  "https://s.kwaixiaodian.com/zone/goods/nexus/self/release/add";
...
const GOODS_DETAIL_IMAGE_UPLOAD_CONTAINER_SELECTOR =
...
async function waitForSubmitAuditSuccess(
  page,
  {
    timeoutMs = SUBMIT_AUDIT_WAIT_TIMEOUT_MS,
    pollingMs = SUBMIT_AUDIT_WAIT_POLL_INTERVAL_MS,
  } = {},
) {
...
async function checkLogin(page) {
  return await isShopPlatformLoggedIn(page, "kuaishou_shop");
}
...
async function fillExactInput(page, selector, value, fieldLabel) {
  try {
    ...
    logger.warn(
      `${PLATFORM_NAME}${fieldLabel}填写失败: ${error?.message || error}`,
    );
    return false;
  }
}
...
async function clearGoodsImages(page) {
  try {
    ...
    const deleteCount = await page.evaluate((selectors) => {
      ...
      selectors.forEach((selector) => {
        ...
        const targets = Array.from(
          container.querySelectorAll('[aria-label="close-circle"]'),
        );
    ...
    logger.info(
      `${PLATFORM_NAME}商品主图逻辑：已执行原图删除，deleteCount=${deleteCount}`,
    );
    return deleteCount;
  } catch (error) {
    logger.warn(
      `${PLATFORM_NAME}商品主图逻辑：原图删除执行失败: ${error?.message || error}`,
    );
    return 0;
  }
}
...
async function scanGoodsUploadContainers(page) {
  return page
    .evaluate(() => {
      ...
      const containers = Array.from(root.children)
        .map((child, index) => {
          if (!(child instanceof HTMLElement)) {
            return null;
          }
          const fileInputCount =
            child.querySelectorAll('input[type="file"]').length;
          return {
            index: index + 1,
            selector: `#GoodsImg > :nth-child(${index + 1})`,
            tagName: child.tagName.toLowerCase(),
            className: child.className || "",
            textSnippet: String(child.textContent || "")
              .replace(/\s+/g, " ")
              .trim()
              .slice(0, 120),
            fileInputCount,
            uploadedCount: child.querySelectorAll('[aria-label="close-circle"]')
              .length,
            loadingCount: child.querySelectorAll(
              '.loading,[class*="loading"],[class*="uploading"],[class*="progress"]',
            ).length,
          };
        })
        .filter(Boolean);
...
async function resolveUploadContainerSelector(
  page,
  { preferredSelector, candidateIndex, logPrefix },
) {
  const scanResult = await scanGoodsUploadContainers(page);
  logger.info(`${PLATFORM_NAME}${logPrefix}：上传区域扫描结果`, scanResult);

  if (preferredSelector) {
    const preferredInfo = scanResult.containers.find(
      (item) => item.selector === preferredSelector,
    );
  ...
  const candidates = scanResult.containers.filter(
    (item) => item.fileInputCount > 0,
  );
  if (candidates[candidateIndex]) {
    logger.info(
      `${PLATFORM_NAME}${logPrefix}：使用动态识别上传区域`,
  ...
  if (candidates.length > 0) {
    ...
    logger.warn(
      `${PLATFORM_NAME}${logPrefix}：未命中预期上传区域，回退到最后一个可上传区域`,
      fallbackCandidate,
    );
  ...
  logger.warn(
    `${PLATFORM_NAME}${logPrefix}：未扫描到任何可上传区域，回退到默认 selector=${preferredSelector || ""}`,
  );
  return preferredSelector;
}
...
async function waitForUploadCompletion(
  page,
  { containerSelector, targetCount, logPrefix },
) {
  const startedAt = Date.now();
  let lastLogAt = 0;
  ...
  while (Date.now() - startedAt < UPLOAD_WAIT_TIMEOUT_MS) {
    const state = await page
      .evaluate((selector) => {
        ...
        const uploadedCount = container.querySelectorAll(
          '[aria-label="close-circle"]',
        ).length;
        return {
          containerFound: true,
          uploadedCount,
          textSnippet: String(container.textContent || "")
            .replace(/\s+/g, " ")
            .trim()
            .slice(0, 200),
        };
  ...
  const finalState = await page
    .evaluate((selector) => {
      ...
      return {
        containerFound: true,
        uploadedCount: container.querySelectorAll('[aria-label="close-circle"]')
          .length,
        htmlSnippet: String(container.outerHTML || "")
          .replace(/\s+/g, " ")
          .slice(0, 600),
      };
    }, containerSelector)
...
async function uploadGoodsImages(page, filePaths) {
  ...
  try {
    ...
    const beforeCount = await page
      .locator(`${containerSelector} [aria-label="close-circle"]`)
      .count()
      .catch(() => 0);
    const fileInput = page
      .locator(`${containerSelector} input[type="file"]`)
      .first();
    await fileInput.waitFor({ timeout: 10000, state: "attached" });
    await fileInput.setInputFiles(filePaths);
    logger.info(
      `${PLATFORM_NAME}商品主图逻辑：已触发商品图上传，beforeCount=${beforeCount}, expectedAdd=${filePaths.length}, containerSelector=${containerSelector}`,
    );
    ...
    logger.info(
      `${PLATFORM_NAME}商品主图逻辑：商品图上传确认完成，uploadedCount=${uploadedCount}, expectedTotal=${beforeCount + filePaths.length}`,
    );
    return Math.max(0, uploadedCount - beforeCount);
  } catch (error) {
    logger.warn(
      `${PLATFORM_NAME}商品主图逻辑：商品图上传失败: ${error?.message || error}`,
    );
    return 0;
  }
}
...
async function uploadGoodsDetailImages(page, filePaths) {
  ...
  try {
    ...
    await fileInput.setInputFiles(filePaths);
    logger.info(
      `${PLATFORM_NAME}商品详情图逻辑：已触发详情图上传，beforeCount=${beforeCount}, expectedAdd=${filePaths.length}, containerSelector=${containerSelector}`,
    );
    logger.info(
      `${PLATFORM_NAME}商品详情图逻辑：上传触发后重新扫描区域`,
      await scanGoodsUploadContainers(page),
    );
    const uploadedCount = await waitForUploadCompletion(page, {
      containerSelector,
      targetCount: beforeCount + filePaths.length,
    ...
    logger.info(
      `${PLATFORM_NAME}商品详情图逻辑：详情图上传确认完成，uploadedCount=${uploadedCount}, expectedTotal=${beforeCount + filePaths.length}`,
    );
    return Math.max(0, uploadedCount - beforeCount);
  } catch (error) {
    logger.warn(
      `${PLATFORM_NAME}商品详情图逻辑：详情图上传失败: ${error?.message || error}`,
    );
    return 0;
  }
}
...
async function clickSubmitAudit(page) {
  try {
      ...
      return nodes
        .map((node, index) => {
          ...
          return {
            ...
            outerHtmlSnippet: String(node.outerHTML || "")
              .replace(/\s+/g, " ")
              .slice(0, 320),
          };
        })
    ...
    const readyState = await readyHandle
      .jsonValue()
      .then(async () => {
    ...
    if (!readyState?.ready) {
      logger.warn(
        `${PLATFORM_NAME}提交流程：等待超时，提交审核按钮仍不可点击`,
        readyState,
      );
      return false;
    }

    const beforeState = await button
      .evaluate((node) => {
        if (!(node instanceof HTMLButtonElement)) {
          return { found: false };
        }
    ...
    if (beforeState.disabled) {
      logger.warn(
        `${PLATFORM_NAME}提交流程：提交审核按钮当前不可点击`,
        beforeState,
      );
      return false;
    }
    ...
    for (let attempt = 1; attempt <= SUBMIT_AUDIT_RETRY_COUNT; attempt += 1) {
      ...
      if (!currentButtonCount) {
        logger.warn(
          `${PLATFORM_NAME}提交流程：第${attempt}次提交前未找到“提交审核”按钮，先等待成功文案确认`,
          {
            waitTimeoutMs: SUBMIT_AUDIT_WAIT_TIMEOUT_MS,
          },
        );
        const submitResult = await waitForSubmitAuditSuccess(page);
        logger.info(`${PLATFORM_NAME}提交流程：按钮缺失后的成功文案检查结果`, {
          attempt,
          ...submitResult,
        });
      ...
      try {
        ...
        logger.warn(
          `${PLATFORM_NAME}提交流程：第${attempt}次常规点击失败，尝试 DOM click`,
          {
        ...
        if (!domClickResult?.clicked) {
          logger.warn(
            `${PLATFORM_NAME}提交流程：第${attempt}次 DOM click 未执行成功`,
            {
              domClickResult,
            },
          );
      ...
      logger.info(`${PLATFORM_NAME}提交流程：已触发“提交审核”按钮点击`, {
        attempt,
        clickMethod,
      });
      ...
      logger.info(`${PLATFORM_NAME}提交流程：提交结果`, {
        attempt,
        ...submitResult,
      });
      ...
      if (attempt < SUBMIT_AUDIT_RETRY_COUNT) {
        logger.warn(
          `${PLATFORM_NAME}提交流程：第${attempt}次提交未成功，等待后重试`,
          {
        ...
        logger.warn(
          `${PLATFORM_NAME}提交流程：${SUBMIT_AUDIT_RETRY_COUNT}次提交均未成功`,
          submitResult,
        );
      }
    }
...
async function fillTableColumnInputs(
  ...
  try {
    const result = await page.evaluate(
      ({ containerSelector, columnKeyword, markerPrefix }) => {
        ...
        const normalizedKeyword = String(columnKeyword || "")
          .trim()
          .toLowerCase();
        const keywordAliases =
          normalizedKeyword === "sku"
        ...
        const headerRoot =
          container.querySelector(".kwaishop-goods-nexus-pc-table-header") ||
          container.querySelector('[class*="table-header"]') ||
          container.querySelector("thead");
        const bodyRoot =
          container.querySelector(".kwaishop-goods-nexus-pc-table-body") ||
        ...
        const headerCells = Array.from(
          (headerTable || headerRoot).querySelectorAll(
            'th,[role="columnheader"],.header-cell,.table-cell,[class*="header-cell"]',
          ),
        );
        ...
        if (columnIndex < 0) {
          throw new Error(
            `未找到“${columnKeyword}”列，headers=${headerCells.map((cell) => String(cell.textContent || "").trim()).join(" | ")}`,
          );
        }

        const headerCell = headerCells[columnIndex];
        const bodyRows = Array.from(
          (bodyTable || bodyRoot).querySelectorAll(
            'tr,[role="row"],.table-row,[class*="table-row"]',
          ),
        ).filter((row) => {
          return (
            row.querySelectorAll(
              'td,[role="cell"],.table-cell,[class*="table-cell"]',
            ).length > columnIndex
          );
        });
        const columnInputs = [];
          ...
          inputs.forEach((input) => {
            if (
              ...
              candidateInputs.push({
                ...
                cellText: String(cell.textContent || "")
                  .trim()
                  .slice(0, 180),
          ...
          fallbackInputs.forEach((input) => {
            if (
              ...
              if (
                ...
                keywordAliases.some((keyword) =>
                  dataLabel.toLowerCase().includes(keyword),
                ) ||
                keywordAliases.some((keyword) =>
        ...
        return {
          ...
          headerTexts: headerCells.map((cell) =>
      ...
      {
        containerSelector,
        columnKeyword: String(columnKeyword || "")
          .trim()
          .toLowerCase(),
        markerPrefix: `yishe-${String(columnKeyword || "column")
          .trim()
          .toLowerCase()}`,
      },
    );
    ...
    logger.info(
      `${PLATFORM_NAME}${logPrefix}：候选输入框明细`,
      result.candidateInputs || [],
    );
    ...
    for (const inputId of result.taggedInputIds || []) {
      ...
      if (elapsedMs > TABLE_FILL_TOTAL_TIMEOUT_MS) {
        logger.warn(
          `${PLATFORM_NAME}${logPrefix}：达到总超时时间，提前结束填写`,
          {
            elapsedMs,
            filledCount,
    ...
    await page
      .evaluate(() => {
        document
          .querySelectorAll("[data-yishe-sku-code-id]")
          .forEach((node) => {
    ...
    logger.warn(
      `${PLATFORM_NAME}${logPrefix}执行失败: ${error?.message || error}`,
    );
    return 0;
  }
}
...
export async function publishToKuaishouShop(publishInfo = {}) {
  ...
  try {
    ...
    const title = normalizeLimitedText(
      publishInfo.title || publishInfo.name || "",
      30,
    );
    const shortTitle = normalizeLimitedText(
      settings.shortTitle ?? publishInfo.shortTitle ?? title,
      10,
    );
    const images = Array.isArray(publishInfo.images)
    ...
    const shortTitleFilled = shortTitle
      ? await fillExactInput(
          page,
          BASE_INFO_SHORT_TITLE_SELECTOR,
          shortTitle,
          "短标题",
        )
      : false;
    ...
    if (images.length > 0) {
      ...
      logger.info(
        `${PLATFORM_NAME}图片已预处理完成，count=${preparedImageCount}`,
      );
      uploadedImageCount = await uploadGoodsImages(
        page,
      ...
      uploadedDetailImageCount = await uploadGoodsDetailImages(
        page,
        preparedImages.filePaths,
      );
    ...
    if (!productCode) {
      ...
      if (productCodeFilledCount <= 0) {
        logger.error(
          `${PLATFORM_NAME}SKU编码逻辑：未成功填写任何 SKU 编码输入框`,
          {
            productCode,
            pageUrl: page.url(),
          },
