...
export function setTokenPersistenceHandlers(
  handlers: TokenPersistenceHandlers,
): void {
...
export function saveToken(newToken: string): void {
  ...
  writeLocalServerLog("INFO", "本地服务 token 已保存", {
    hasToken: !!newToken,
  });
}
    ...
    data?.user && typeof data.user === "object"
      ? {
          ...
          nickname: data.user.nickname
            ? String(data.user.nickname).trim()
            : null,
    ...
    data?.machine && typeof data.machine === "object"
      ? {
          ...
          platform: data.machine.platform
            ? String(data.machine.platform).trim()
            : null,
          createdAt: data.machine.createdAt
            ? String(data.machine.createdAt).trim()
            : null,
        }
      : undefined;
...
async function _startServer(port: number = 1519): Promise<() => Promise<void>> {
  ...
  app.get("/api/auth/session", async (_req, res) => {
    try {
      ...
      const identity = authorized
        ? await getCurrentUserIdentity().catch(() => ({
            userId: "",
            account: "",
          }))
        : { userId: "", account: "" };
  ...
  ioServer.on("connection", (socket) => {
    ...
    const clientSource =
      (socket.handshake.query.clientSource as string) || "unknown";
  ...
  app.get("/api/queue/tasks", async (req, res) => {
    try {
      ...
      const response = await fetch(
        `${REMOTE_API_BASE}/queue/messages?${queryParams.toString()}`,
        {
          method: "GET",
          headers: {
    ...
    try {
      ...
      if (
        ...
        res.status(400).json({
          ...
          status: false,
          message:
            "状态值无效，必须是 pending、processing、completed 或 failed 之一",
        });
        return;
      }
  ...
  const handleMaterialUpload = async (
    ...
    const target = normalizeMaterialUploadTarget(
      req.body?.target,
      fallbackTarget,
    );

    console.log(
      `[material-upload:${requestId}] ========== 开始处理请求 ==========`,
    );
    const requestLogPayload = { ...req.body, target };
    if (
      typeof requestLogPayload.imageData === "string" &&
      requestLogPayload.imageData
    ) {
    ...
    try {
      ...
      if (!url || typeof url !== "string") {
        console.error(
          `[material-upload:${requestId}] 参数验证失败: url 参数必填`,
        );
        res.status(400).json({
          code: 1,
          status: false,
      ...
      try {
        ...
        if (result.ok) {
          console.log(
            `[material-upload:${requestId}] ========== ✅ 全部流程完成 ==========`,
          );
          console.log(`[material-upload:${requestId}] 上传目标: ${target}`);
          console.log(
            `[material-upload:${requestId}] COS URL: ${result.data?.cosUrl}`,
          );
          console.log(
        ...
        console.error(
          `[material-upload:${requestId}] ❌ 上传失败: ${result.message}`,
        );
        res.status(500).json({
          code: 1,
          status: false,
        ...
        console.error(
          `[material-upload:${requestId}] ❌ IPC调用失败:`,
          ipcError,
        );
        res.status(500).json({
          code: 1,
  ...
  app.post("/api/file-upload", requireLocalAuth, async (req, res) => {
    ...
    if (
      typeof requestLogPayload.fileData === "string" &&
      requestLogPayload.fileData
    ) {
      requestLogPayload.fileData = `[data-url omitted, length=${requestLogPayload.fileData.length}]`;
    }
    if (
      typeof requestLogPayload.localFilePath === "string" &&
      requestLogPayload.localFilePath
    ) {
      requestLogPayload.localFilePath = "[local path omitted]";
    }

    console.log(
      `[file-upload:${requestId}] ========== 开始处理请求 ==========`,
    );
    console.log(
      `[file-upload:${requestId}] 请求体:`,
      JSON.stringify(requestLogPayload, null, 2),
    );
    ...
    try {
      ...
      const hasLocalFilePath = Boolean(
        localFilePath && typeof localFilePath === "string",
      );
      ...
      if (result.ok) {
        console.log(
          `[file-upload:${requestId}] ========== ✅ 全部流程完成 ==========`,
        );
        console.log(
          `[file-upload:${requestId}] COS URL: ${result.data?.cosUrl}`,
        );
        res.status(200).json({
          code: 0,
          status: true,
