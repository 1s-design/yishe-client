...
const searchDef: CapabilityDefinition = {
  ...
  riskLevel: 'read',
  argsSchema: z.object({
    keyword: z.string().describe('搜索关键词 (如 street photography, wildlife, vintage)'),
  ...
  handler: async ({ keyword, limit, page }) => {
    ...
    const result = await searchFlickr(keyword.trim(), { limit, page });
    return {
...
const downloadDef: CapabilityDefinition = {
  ...
  riskLevel: 'write',
  argsSchema: z.object({
    imageUrl: z.string().describe('Flickr 图片直链'),
...
const collectDef: CapabilityDefinition = {
  ...
  handler: async ({ keyword, maxCount, syncToMaterial, imageUrl, title, metadata }) => {
    ...
    const searchRes = await searchFlickr(keyword.trim(), { limit });
