...
const searchDef: CapabilityDefinition = {
  ...
  riskLevel: 'read',
  argsSchema: z.object({
    keyword: z.string().optional().describe('搜索关键词 (如 vintage poster, nature, coffee)'),
  ...
  handler: async (args: { keyword?: string; query?: string; limit?: number; pageSize?: number; page?: number }) => {
    ...
    const result = await searchDuckDuckGo(keyword, { limit, page });
    return {
...
const downloadDef: CapabilityDefinition = {
  ...
  riskLevel: 'write',
  argsSchema: z.object({
    imageUrl: z.string().optional().describe('DuckDuckGo 图片直链'),
...
const collectDef: CapabilityDefinition = {
  ...
  handler: async (args: {
    ...
    const searchRes = await searchDuckDuckGo(keyword, { limit });
