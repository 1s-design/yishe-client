...
const searchDef: CapabilityDefinition = {
  ...
  riskLevel: 'read',
  argsSchema: z.object({
    keyword: z.string().optional().describe('搜索关键词 (如 海报背景, 唯美风景, 卡通)'),
  ...
  handler: async (args: { keyword?: string; query?: string; limit?: number; pageSize?: number; page?: number }) => {
    ...
    const result = await searchSo(keyword, { limit, page });
    return {
...
const downloadDef: CapabilityDefinition = {
  ...
  riskLevel: 'write',
  argsSchema: z.object({
    imageUrl: z.string().optional().describe('360 图片直链'),
...
const collectDef: CapabilityDefinition = {
  ...
  handler: async (args: {
    ...
    const searchRes = await searchSo(keyword, { limit });
