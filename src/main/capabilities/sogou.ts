...
const searchDef: CapabilityDefinition = {
  ...
  riskLevel: 'read',
  argsSchema: z.object({
    keyword: z.string().describe('搜索关键词 (如 极简插画, 萌宠, 国风)'),
  ...
  handler: async ({ keyword, limit, page }) => {
    ...
    const result = await searchSogou(keyword.trim(), { limit, page });
    return {
...
const downloadDef: CapabilityDefinition = {
  ...
  riskLevel: 'write',
  argsSchema: z.object({
    imageUrl: z.string().describe('搜狗图片直链'),
...
const collectDef: CapabilityDefinition = {
  ...
  handler: async ({ keyword, maxCount, syncToMaterial, imageUrl, title, metadata }) => {
    ...
    const searchRes = await searchSogou(keyword.trim(), { limit });
