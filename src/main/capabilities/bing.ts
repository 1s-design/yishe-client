/**
 * 客户端通用能力 — 必应图片搜索与采集 (Bing Images)
 * 注册到 Capability Registry 后，自动暴露为:
 *   1. REST API  POST /api/capabilities/bing/search|download|collect|status
 *   2. MCP 工具  bing_search / bing_download / bing_collect / bing_status
 */
import { z } from 'zod';
...
import { searchBing, getBingStatus, downloadBingImage, syncBingToMaterialLibrary } from '../bing';

const searchDef: CapabilityDefinition = {
  name: 'search',
  namespace: 'bing',
  description: '在必应 (Bing) 搜索全球高质量图片素材，返回原图直链与长宽信息。',
  riskLevel: 'read',
  argsSchema: z.object({
    keyword: z.string().optional().describe('搜索关键词 (如 landscape, cat, modern minimalist)'),
  ...
  handler: async (args: { keyword?: string; query?: string; limit?: number; pageSize?: number; page?: number }) => {
    ...
    const result = await searchBing(keyword, { limit, page });
    return {
...
const downloadDef: CapabilityDefinition = {
  name: 'download',
  namespace: 'bing',
  description: '下载单张必应图片到本地临时路径。',
  riskLevel: 'write',
  argsSchema: z.object({
    imageUrl: z.string().optional().describe('必应图片直链'),
  ...
  handler: async (args: { imageUrl?: string; image?: string; url?: string; filename?: string }) => {
    ...
    const res = await downloadBingImage(imageUrl, {
      filename: args.filename,
      destDir: path.join(os.tmpdir(), 'bing'),
    });
...
const collectDef: CapabilityDefinition = {
  name: 'collect',
  namespace: 'bing',
  description: '将必应图片下载并同步至素材库（支持单图入库或批量搜索入库）。',
  riskLevel: 'write',
  argsSchema: z.object({
  ...
  handler: async (args: {
    ...
    // 1. 单图精准入库模式
    if (imageUrl) {
      const syncRes = await syncBingToMaterialLibrary(imageUrl, {
        title: args.title,
        ...(args.metadata || {}),
      });
      return {
    ...
    const searchRes = await searchBing(keyword, { limit });
    if (!searchRes.success || !searchRes.items.length) {
      return { success: false, error: searchRes.error || '未检索到可用必应图片' };
    }
    ...
    for (const item of searchRes.items) {
      ...
      try {
        const syncRes = await syncBingToMaterialLibrary(item.image, {
          title: item.title,
...
export function registerBingCapabilities(): void {
