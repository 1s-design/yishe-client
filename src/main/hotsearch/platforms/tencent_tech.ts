/**
 * 腾讯科技热搜
 * 数据源：腾讯科技频道页面 HTML 解析
 */

import type { PlatformModule } from '../types'
import axios from 'axios'

const tencentTech: PlatformModule = {
  config: {
    key: 'tencent_tech',
    name: '腾讯科技',
    enabled: true,
  ...
  async fetch(ctx) {
    const { data: html } = await axios.get('https://news.qq.com/ch/tech', {
      timeout: ctx.timeout,
      responseType: 'text',
      headers: {
    ...
    // 腾讯科技链接格式: /rain/a/XXXXXXXX
    const linkRegex = /<a[^>]*href="([^"]*rain\/a\/[^"]*)"[^>]*target="[^"]*_blank[^"]*"[^>]*>([^<]{8,120})<\/a>/gi
    ...
    // 若未命中，尝试通用链接提取
    if (items.length === 0) {
      const genericRegex = /<a[^>]*href="(\/rain\/a\/[^"]+)"[^>]*>([^<]{8,120})<\/a>/gi
