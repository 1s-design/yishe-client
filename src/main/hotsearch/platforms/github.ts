/**
 * GitHub Trending 平台模块
 *
 * 注意：核心采集逻辑已迁移至服务端能力定义（hotsearch-github.capability.ts）
...
async function executeViaServer(params: Record<string, any>): Promise<any> {
  ...
  const endpoint = `${serverUrl}/api/workflow/node-capabilities/hotsearch_github/execute`;
...
const github: PlatformModule = {
  ...
  async fetch(ctx) {
    ...
    if (executionMode === 'client') {
      result = await DynamicCapabilityManager.executeCapability('hotsearch_github', { maxCount });
    } else {
