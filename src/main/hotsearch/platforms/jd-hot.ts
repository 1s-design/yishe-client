 ...
 * 京东热搜平台模块
 *
 * 注意：核心采集逻辑已迁移至服务端能力定义（hotsearch-jd-hot.capability.ts）
...
async function executeViaServer(params: Record<string, any>): Promise<any> {
  ...
  const endpoint = `${serverUrl}/api/workflow/node-capabilities/hotsearch_jd_hot/execute`;
...
const jdHot: PlatformModule = {
  ...
  async fetch(ctx) {
    ...
    if (executionMode === 'client') {
      result = await DynamicCapabilityManager.executeCapability('hotsearch_jd_hot', { maxCount });
    } else {
