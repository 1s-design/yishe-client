...
export interface ClaimNextPublishTaskResponse {
  success: boolean;
  claimed: boolean;
  reason?: string;
  message?: string;
...
// 客户端主动领取发布任务
export const claimNextPublishTask = async (data: {
  clientId?: string;
  machineCode?: string;
  profileId?: string;
}): Promise<ClaimNextPublishTaskResponse> => {
  const response = await request.post({
    url: "/websocket/publish-tasks/claim-next",
    data,
    timeout: 30000,
  });
  return unwrapApiData<ClaimNextPublishTaskResponse>(response, {
    success: false,
    claimed: false,
...
export const recoverClientPublishTaskOrphans = async (data: {
  clientId?: string;
  machineCode?: string;
  profileId?: string;
  reason?: string;
  ...
  return request.post({
    url: "/websocket/publish-tasks/recover-client-orphans",
    data,
    timeout: 15000,
  });
};
