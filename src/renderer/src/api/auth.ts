...
const getTokenTerminalType = (token: string): "admin" | "client" | null => {
  try {
    ...
    return decoded?.terminalType === "client"
      ? "client"
      : decoded?.terminalType === "admin"
        ? "admin"
        : null;
  } catch (error) {
    console.warn("解析 token terminalType 失败:", error);
    return null;
  }
};
...
export const login = async (params: LoginParams): Promise<LoginResponse> => {
  ...
  if (existingToken) {
    const existingTerminalType = getTokenTerminalType(existingToken);
    if (existingTerminalType === "client") {
      console.log("登录前检测到已有 client token，先验证有效性:", {
        hasToken: true,
      ...
      if (isValid) {
        console.log("已有 client token 有效，直接使用，不调用登录接口");
        ...
        console.log("已有 client token 无效，需要重新登录");
      }
    } else {
      console.log("检测到非 client token 或旧 token，清除后重新登录", {
        existingTerminalType,
      });
    }
