 ...
 * 本模块只定义「当前客户端能执行」的任务类型：在此注册类型 + 执行器即可。
 * 展示文案统一从 platform-executors 的平台能力表读取。
 */

import { executePlatformTask, getPlatformCapabilityByTaskType } from './platform-executors'
...
export function getExecutableTaskDisplayList(): { value: string; label: string }[] {
  return EXECUTABLE_TASK_TYPES.map((value) => ({
    value,
    label: getPlatformCapabilityByTaskType(value)?.label ?? value,
  }))
}
...
/** 淘宝发布处理 */
async function executeTaobaoPublish(row: any, context?: PlatformTaskExecutionContext) {
